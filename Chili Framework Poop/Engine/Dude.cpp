...
void Dude::Move()
{
	...
	if (x + w >= Graphics::ScreenWidth)
	{
		x =  (Graphics::ScreenWidth - 1) - Dude::w;
	}
	if (y < 0)
	{
		y= 0;
	}
	if (y + Dude::h >= Graphics::ScreenHeight)
	{
		y =  (Graphics::ScreenHeight - 1) - Dude::h;
	}
...
void Dude::UpdateMouse(const Mouse & m)
{
	if (m.LeftIsPressed())
	{
		x = m.GetPosX();
